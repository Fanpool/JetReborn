    ...
    public static void onClientTick(MinecraftClient client) {
        ...
        if (mc.player != null && mc.world != null) {
            if (!mc.isPaused()) {
                ...
                if (!chest.isEmpty() && item instanceof Jetpack jetpack && JetpackUtils.isFlying(mc.player)) {
                    if (JetRebornConfig.enableJetpackParticles && (mc.options.particles != ParticlesMode.MINIMAL)) {
                        ...
                        Vec3d v = playerPos.add(vLeft).add(mc.player.getVelocity().multiply(jetpack.getSpeedSide()));
                        ...
                        v = playerPos.add(vRight).add(mc.player.getVelocity().multiply(jetpack.getSpeedSide()));
                        mc.particleManager.addParticle(ParticleTypes.FLAME, v.x, v.y, v.z, random, -0.2D, random);
                        mc.particleManager.addParticle(ParticleTypes.SMOKE, v.x, v.y, v.z, random, -0.2D, random);
                    }

                    if (JetRebornConfig.enableJetpackSounds && !JetpackSound.playing(mc.player.getId())) {
